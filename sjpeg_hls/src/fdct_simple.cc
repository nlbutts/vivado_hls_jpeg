...
void fdct_buffer(int16_t coeffs[64])
 {
    ColumnDct(coeffs);
...
void DCT8a(int16_t a[8], uint8_t stride)
{
    const double pi = 3.1415926535897932384626433832795;
    const int16_t m1 = (std::cos(4*pi/16) * 32768) + 0.5;
    const int16_t m2 = (std::cos(6*pi/16) * 32768) + 0.5;
    const int16_t m3 = ((std::cos(2*pi/16) - std::cos(6*pi/16)) * 32768) + 0.5;
    const int16_t m4 = ((std::cos(2*pi/16) + std::cos(6*pi/16)) * 16384) + 0.5;

    int16_t b0, b1, b2, b3, b4, b5, b6, b7;
    int16_t c0, c1, c2, c3, c4, c5, c6;
    int16_t d0, d1,     d3, d4;
    int16_t e2, e3, e4,     e6, e7;
    int16_t f2, f3, f4, f5, f6, f7;

    /* Step 1 */
    b0 = a[stride * 0] + a[stride * 7];
    b1 = a[stride * 1] + a[stride * 6];
    //b2 = a[stride * 3] - a[stride * 4]; /* corrected */
    b2 = a[stride * 2] - a[stride * 4]; /* corrected */
    b3 = a[stride * 1] - a[stride * 6];
    b4 = a[stride * 2] + a[stride * 5];
    b5 = a[stride * 3] + a[stride * 4];
    b6 = a[stride * 2] - a[stride * 5];
    b7 = a[stride * 0] - a[stride * 7];  /***/
...
void fdct_buffer2(int16_t coeffs[64])
{
	int16_t * ptr = coeffs;
    fdct_buffer2_label1:for (ap_uint<4> i = 0; i < 8; i++)
    {
        DCT8a(coeffs + i, 8);
    }
    fdct_buffer2_label2:for (ap_uint<4> i = 0; i < 8; i++)
    {
        DCT8a(ptr, 1);
        ptr += 8;
    }
    fdct_buffer2_label0:for (ap_uint<7> i = 0; i < 64; i++)
    {
        coeffs[i] <<= 1;
    }
...
void SjpegFdctC_simple(int16_t coeffs[384], int num_blocks)
{
    //while (num_blocks-- > 0)
    SjpegFdctC_simple_label3:for (ap_uint<3> i = 0; i < 6; i++)
    {
        fdct_buffer2(coeffs);
        coeffs += 64;
    }
